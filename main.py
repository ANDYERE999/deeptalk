...
不过，请注意其他可能窃取你聊天记录的方式(尤其是输入法)
    ...
    def _show_about_window(self):
        ...
        本工具是一个完全在您本地电脑上运行的非对称加密聊天辅助工具。
        它能帮助您在任何不安全的网络环境中，与您的朋友进行无法被第三方（包括网络服务提供商、应用后台等）破译的私密对话。

        【核心理念】

        在数字时代，隐私不应成为一种奢侈品。
        本工具的开发初衷，就是为普通用户提供一个简单、可靠、无需信任任何第三方的加密方案，将数据的所有权和保密权，真正交还到用户自己手中。

        【安全与技术】

        - 加密标准：采用业界成熟的 RSA-2048 位非对称加密算法，并结合 PKCS1_OAEP 填充方案，确保了极高的加密强度和安全性。

        - 开源可信：本应用完全开源，所有代码均可被公开审查，以证明其不含任何后门或恶意行为。

        - 本地运行：所有密钥的生成、加密和解密过程，均在您的电脑本地完成，不依赖任何网络连接，从根本上杜绝了密钥在传输过程中被窃取的风险。

        【致谢】

        本应用基于 Python 及以下优秀的开源库构建：
        - Tkinter (图形界面)
        - PyCryptodome (核心加密算法)
        - Pyperclip (剪贴板功能)
        ...
        开发者: 冬雪莲
        联系方式/项目地址: https://github.com/winter-snow-lotus/asymmetric-chat-app
        """
